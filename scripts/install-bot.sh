...
case "$(uname -s)" in
  ...
  *)
    ...
    echo "Windows users: download nito-bot-windows-amd64.exe from https://github.com/${REPO}/releases" >&2
    exit 1
    ;;
esac
...
ASSET_NAME="nito-bot-${OS}-${ARCH}"

echo "Fetching latest nito-bot release..."
...
echo "Installing nito-bot to ${INSTALL_DIR}/${BINARY}..."
mv "${TMPDIR}/${ASSET_NAME}" "${INSTALL_DIR}/${BINARY}"

echo ""
echo "nito-bot ${LATEST_TAG} installed."
echo "Run it with: nito-bot -f bot.yml -s <scripts-dir>"
echo "See BOTS.md for the bot.yml schema and bootstrap walk-through."
